...
const SocialConnectionsClient = () => {
    ...
    // Note: 'mutateConnectionsCount' is the SWR function to re-fetch
    ...
    // This is false if maxConnections is 0 (loading) or if current < max
    ...
    const handleDisconnect = async (platform) => {
        ...
        try {
            ...
            await mutateConnectionsCount(); // Re-fetch the total count from SWR
            // --- End Refresh ---
                    ...
                    <div key={platform} className="py-4 sm:flex sm:items-center sm:justify-between bg-white p-4 rounded-lg shadow-sm">
                        ...
                        <div className="mt-2 sm:mt-0 flex items-center space-x-4">
                            ...
                            <Switch
                                ...
                                } relative inline-flex h-6 w-11 flex-shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500`}
                            >
                                <span className="sr-only">Enable {config.name}</span>
                                <span
                                    aria-hidden="true"
                                    className={`${
