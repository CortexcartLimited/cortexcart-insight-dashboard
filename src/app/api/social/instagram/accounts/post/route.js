...
export async function POST(req) {
    ...
    try {
        ...
        const createContainerResponse = await axios.post(`https://graph.facebook.com/v19.0/${instagramUserId}/media`, { image_url: absoluteImageUrl, caption: caption, access_token: accessToken });

        const creationId = createContainerResponse.data.id;
        if (!creationId) throw new Error('Failed to create media container.');

        await axios.post(`https://graph.facebook.com/v19.0/${instagramUserId}/media_publish`, { creation_id: creationId, access_token: accessToken });
