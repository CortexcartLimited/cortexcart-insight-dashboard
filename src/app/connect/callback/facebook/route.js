...
export async function GET(req) {
    ...
    try {
        console.log(`[FACEBOOK CALLBACK] User ${userEmail}: Starting token exchange.`);
        ...
        console.log(`[FACEBOOK CALLBACK] User ${userEmail}: Token received. Fetching pages.`);
        ...
        console.log('[FACEBOOK CALLBACK] RAW DATA RECEIVED FROM FACEBOOK:');
        console.log(JSON.stringify(pagesData, null, 2));
        // ------------------------------------

        await db.query(
            `INSERT INTO social_connect (user_email, platform, access_token_encrypted) VALUES (?, 'facebook', ?) ON DUPLICATE KEY UPDATE access_token_encrypted = VALUES(?)`,
            [userEmail, encrypt(userAccessToken), encrypt(userAccessToken)]
        );
